...
public class OrderDaoImpl implements OrderDao {
    ...
    private static final String ROLLBACK_ERR_MSG = "Failed to release resources";
    private static final String READ_ALL = "SELECT order_id, order_date, customer_id, drug_id, amount FROM order LEFT JOIN order_m2m_drug USING (order_id) ORDER BY order_id";
    private static final String READ_BY_ORDER_ID = "SELECT order_id, order_date, customer_id, drug_id, amount FROM order_m2m_drug LEFT JOIN order USING (order_id) WHERE order_id = ?";
    private static final String INSERT = "INSERT INTO mydb.order (order_date, customer_id) VALUES (?,?)";
    private static final String DELETE = "DELETE FROM order_m2m_drug LEFT JOIN order USING (order_id) WHERE order_id = ?";
    private static final String READ_BY_USER_ID = "SELECT order_id, order_date, customer_id, drug_id, amount FROM order_m2m_drug LEFT JOIN mydb.order USING (order_id) WHERE customer_id = ? ORDER BY order_id";
    ...
    public int create(Order entity) throws DaoException {
        try{
            ...
            parameters[0] = new Object[]{entity.getOrderDate(), entity.getCustomerId()};
            ...
            return jdbcHelper.executeMassiveUpdate(sqlRequests,parameters);
        } catch (SQLException | DaoException e){
            LOGGER.error(e);
            throw new DaoException(ROLLBACK_ERR_MSG, e);
        }
    }
